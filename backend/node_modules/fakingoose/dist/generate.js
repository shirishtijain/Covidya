...
const generators = {
    string: (pathDef, { options }) => {
        ...
        if (types_1.isStringFieldOptions(options)) {
            switch (options.type) {
                case 'email':
                    return chance.email();
                case 'firstname':
    ...
    objectid: (pathDef, { options, globalOptions }) => {
        if (types_1.isPopulateWithFactory(options)) {
            return options.populateWithFactory.generate();
        }
        else if (types_1.isPopulateWithSchema(options)) {
            const mockGen = mocker_1.factory(options.populateWithSchema);
